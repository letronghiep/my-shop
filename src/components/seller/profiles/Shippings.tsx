...
import { IDataMessage, IShipping } from "@/types/global";
...
import LocationSelect, { IDataLocation } from "../../inputs/LocationSelect";
import ShippingItem from "./ShippingItem";
import { Flex } from "antd";
const MapNoSSR = dynamic(() => import("../../Map"), { ssr: false });

interface IShippingData extends IShipping {
  detail_address?: string;
}
interface IResponseShipping {
  shippingData: IDataMessage;
  error: AxiosError;
...
function Shippings({
  ...
  const [dataLocation, setDataLocation] = useState<IDataLocation>();
  const [shippingId, setShippingId] = useState<string>("");
  const [shippingDetail, setShippingDetail] = useState<IShipping | null>(null);
  ...
  const handleChangeLocation = (location: IDataLocation) => {
    const {
  ...
  const bodyContent = (
    ...
    >
      ...
      <Input
        ...
        control={control}
      />
      {/* <MapNoSSR lat={dataPosition.lat} lng={dataPosition.lng} /> */}
      <div>
        <CheckboxCustom
  ...
  const onSubmit = async (data: IShippingData) => {
    try {
      setLoading(true);
  ...
  return (
    <div className="bg-white p-6">
      ...
      {/* <Modal
        ...
        title="Thêm điểm giao hàng"
        body={bodyContent}
        footer={<div></div>}
        actionLabel={`${shippingId}` ? "Cập nhật" : "Hoàn thành"}
        secondaryAction={handleOpenModal}
        secondaryActionLabel="Trở lại"
        disabled={loading}
      /> */}
      <Modal
        open={openModal}
